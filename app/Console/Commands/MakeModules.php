...
class MakeModules extends Command
{
    ...
    //в папке HTTP
    public const PRESINTATION_HTTP = ['Graphql', 'Controllers'];
    ...
    private function createInnerFolder(string $path, string $folder, array $static)
    {
        ...
        foreach ($static as $value) {
            ...
            if($value == self::PRESINTATIONFOLDER[2])
            {
                foreach (self::PRESINTATION_HTTP as $value_inner) {
                    File::makeDirectory($pathIn . '/' . $value . '/' . $value_inner , 0777, true);
                }
            }
